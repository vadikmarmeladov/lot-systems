...
import { COUNTRY_BY_ALPHA3, DATE_FORMAT, DATE_TIME_FORMAT, LOG_MESSAGE_STALE_TIME_MINUTES, MAX_LOG_TEXT_LENGTH, MAX_SYNC_CHAT_MESSAGE_LENGTH, SYNC_CHAT_MESSAGES_TO_SHOW, USER_SETTING_NAMES, WEATHER_STALE_TIME_MINUTES, } from '#shared/constants';
...
export default async (fastify) => {
    ...
    fastify.post('/settings', async (req, reply) => {
        ...
        const body = fp.pick(USER_SETTING_NAMES)(req.body);
        if (body.country) {
        ...
        process.nextTick(async () => {
            ...
            if (body.city && body.country) {
                const coordinates = await weather.getCoordinates(body.city, body.country);
                if (coordinates) {
                    newTimeZone = await weather.getTimeZone(coordinates.lat, coordinates.lon);
                }
            }
    ...
    fastify.get('/live-message', async (req, reply) => {
    ...
    fastify.post('/chat-messages/like', async (req, reply) => {
        const message = await fastify.models.ChatMessage.findByPk(req.body.messageId);
        if (!message)
    ...
    fastify.get('/logs', async (req, reply) => {
        const logs = await fastify.models.Log.findAll({
            ...
            order: [['createdAt', 'DESC']],
        }).then((xs) => xs.filter((x, i) => x.event !== 'note' || x.text.length || i === 0));
    ...
    fastify.get('/memory', async (req, reply) => {
        ...
        function getPeriodEdges(inputDate) {
        ...
        if (prevQuestionIds.length) {
            untouchedQuestions = defaultQuestions.filter(fp.propNotIn('id', prevQuestionIds));
            if (!untouchedQuestions.length) {
                const longAgoAnsweredQuestionIds = prevQuestionIds.slice(-1 * Math.floor(prevQuestionIds.length / 3));
                untouchedQuestions = defaultQuestions.filter(fp.propIn('id', longAgoAnsweredQuestionIds));
            }
        }
        const rng = seedrandom(`${req.user.id} ${localDate.format(DATE_FORMAT)} ${isNightPeriod ? 'N' : 'D'}`);
        const question = untouchedQuestions[Math.floor(rng() * untouchedQuestions.length)];
        return question;
    });
    fastify.post('/memory/answer', async (req, reply) => {
