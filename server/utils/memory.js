...
import { COUNTRY_BY_ALPHA3, USER_SETTING_NAME_BY_ID, } from '#shared/constants';
...
export async function completeAndExtractQuestion(prompt) {
...
export async function buildPrompt(user, logs) {
    ...
    if (localDate && context.city && context.country) {
        ...
        if (context.temperature && context.humidity) {
            contextLine += `, with a current temperature of ${Math.round(toCelsius(context.temperature))}℃ and humidity at ${Math.round(context.humidity)}%.`;
        }
        else {
            contextLine += '.';
        }
...
function formatLog(log) {
    ...
    const country = date && log.context.country
        ? COUNTRY_BY_ALPHA3[log.context.country]?.name || ''
        : '';
    const temperature = date && log.context.temperature
        ? `T:${Math.round(toCelsius(log.context.temperature))}℃`
        : '';
    const humidity = date && log.context.humidity ? `H:${Math.round(log.context.humidity)}%` : '';
    return [
        `---`,
        [
