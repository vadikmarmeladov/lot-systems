...
NOTE (2026-08-29, LOT-SR-20260829-01): the prior "COSMO Hardware |
brave-lamport-t9z5u8 | c7d353ef | complete hardware computer design v1.0"
row's branch and commit hash do not resolve anywhere in this repository's
git history (`git cat-file -t c7d353ef` fails; no local or remote ref named
brave-lamport-t9z5u8 exists). That catalog entry outlived its content. It is
replaced above by "COSMO Terminal," the first hardware-computer spec with an
actual body: docs/corporate/LOT-COSMO-COMPUTER-v1.md + BOM + firmware/
software docs + PDF manual, on this session's own branch.
...
COSMO Terminal   | claude/brave-lamport-15udnr  | (pending)| 1/1   | READY  | 5     | +~900  | COSMO® Terminal T-1 — hardware computer plan/BOM/firmware/software/manual (SEE NOTE)
