...
20260824-01    | ENGINEERING | Log /story write→read loop fix: generated_story added to displayableEvents whitelist (was silently dropped, WIKI-GUARD-style precedent) · Logs.tsx STORY [RFI]: render case added · Field Manual: /story /system /how documented (were undocumented despite being fully shipped) · tsconfig.server.json ignoreDeprecations 5.0→6.0 (tsc 6.0.2 env drift) · sandboxed build env: node_modules absent, registry.npmjs.org + registry.yarnpkg.com both 403 at egress proxy — static tsc/prettier verification substituted, PROVISIONAL until full build runs | GREEN (STATIC-VERIFIED) | 70c1b1ad (tag push 403'd — branch scope only) | WORDS: 803 (median 576) — trend ↑
