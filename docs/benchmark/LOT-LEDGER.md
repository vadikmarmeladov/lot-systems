...
20260823-01    | ENGINEERING | LOT Mail — /email to <Name> Log trigger, name resolution (POST /api/users/search), Sync "✉️ LOT Mail" inbox (GET /api/direct-messages), wired dormant /dm/:userId + DirectMessageThread into an actual entry point, Cohort "Send message" -> goTo('dm') fix. Found+fixed 2 blocking bugs in pre-existing DM scaffolding: missing direct_messages migration; SSE relay switch missing 'direct_message' case (silent-drop pattern, same class as SR-20260630-01 formatLog() clause) | GREEN | (pending tag) | WORDS: 921 (median ~540)
