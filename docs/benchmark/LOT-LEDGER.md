...
20260817-01     | ENGINEERING  | Calendar widget: optional per-entry TIME field · due-time sweep (30s, doc.hidden-guarded) · calendar_alert Log event deduped against the Log stream itself · military ALERT: banner (reuses .blink + existing opacity/tabular-nums idiom, no new UI infra) · Logs.tsx ALERT: handler · selfAssembly calendar_alert->calendar mapping | GREEN  | (pending)  | WORDS: 640 (median 627) — trend =
