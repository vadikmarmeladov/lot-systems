...
20260722-01    | ENGINEERING | LOT Mail shipped — /email to <name> [msg] Log trigger · resolves recipient by name (CONCAT firstName+lastName ILIKE) · Cohort-aware disambiguation via findCohortMatches (scoped to candidates) · rides existing DirectMessage model + sync.emit, no new table · fixed missing direct_message case in /api/sync SSE relay switch (silently undelivered since the DM feature shipped) · Sync tab live mail banner · 4 prior planning-stage mentions (SR-20260605-01/02, 20260606-02, 20260612-06) never reached master until now | GREEN | (pending tag) | LINES: 256 (5 files)
