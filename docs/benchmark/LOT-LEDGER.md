...
20260811-01    | ENGINEERING  | LOT Email v1 — /email in Log (Logs.tsx trigger 'lot-email') resolves recipient via existing cohort matches, sends through pre-existing DirectMessage rail (POST /api/direct-messages, unmodified), live in Sync.tsx as new "✉️ LOT Email" section off the pre-existing `direct_message` SSE event · zero server/migration changes | GREEN  | 7a3b1e7 | WORDS: ~610 (median 540)
