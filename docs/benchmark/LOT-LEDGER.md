...
20260820-01    | ENGINEERING | LOT® Email — /email to <name> composed in Log (fires on 7s autosave debounce, sends once per recipient), POST /log-email resolves recipient by firstName, posts ChatMessage to Sync via existing SSE bus, mirrors Log event, delivers real mail via existing Resend sendEmail(). Cohort Connect "Send message" rewired to prefill the command (pendingLogInsert one-shot store) — the Cohort Dating / LOT Community integration point. No new DB table. | GREEN | (pending tag) | WORDS: 914 (median 449) — trend ↑
