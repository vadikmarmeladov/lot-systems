...
20260811-01    | ENGINEERING  | LOT-FM-001 BASIC ration module — M1 OPEN TAB cherry-picked from beautiful-johnson-56p7ov + re-wired onto TabPanels; M2 UPGRADE/ROSTER state machine (PENDING→ON_STRENGTH→STEADY_STATE, STAND DOWN, SIMULATED billing in user.metadata); M3 issue engine (itemsForIssue cadence, private COGS/margin gate ≥60%/≤$40, printable manifest card); fixed unanchored server/ .gitignore rule silently swallowing new src/server/ files | GREEN  | f4c82090 | WORDS: ~1220 (median 540) — trend ↑
