...
20260727-01    | ENGINEERING | LOT Mail rebuilt from manifest description (orig. branch fa622a25 unrecoverable): /email(/mail) Log trigger -> GET /api/users/lookup -> existing DirectMessage; Sync Mail inbox (GET /api/direct-messages, SSE-live); CohortConnectWidget "Send message" stub wired to real /dm/:userId thread. MANIFEST-GROUND-TRUTH doctrine clause added (06-27 merge-assumption was wrong for this branch). No new dating/matching logic — scoped to existing Cohort matches only. | GREEN | 169436f2 | WORDS: 410 (median 540) — trend ↓
