...
20260721-01    | ENGINEERING | Calendar Alerts shipped to CalendarWidget: ported gifted-lovelace-cZOWR design (live clock, T-minus/T-plus status machine, military alert overlay, Today panel) onto current master's CalendarWidget · calendar_alert_fired event + CAL-ALERT: COCKPIT-RULE handler in Logs.tsx · both new intervals gated document.hidden/isRouteActive('system') per Render Isolation · caught+fixed missing calendar_alert_fired entry in GET /api/logs displayableEvents whitelist (Backend Whitelist Hygiene, 2nd occurrence) · MANIFEST Calendar Alerts row repointed to this branch, gifted-lovelace-cZOWR marked SUPERSEDED | GREEN | (pending tag) | WORDS: ~560 (median 540)
20260726-01    | ENGINEERING | Calendar Alerts delivered to S-2-designated branch claude/dreamy-babbage-be5mdm: cherry-picked e6faa63c+a82ff3dc from dreamy-babbage-nebo88 (identical add997e6 fork point, zero conflicts) · independently re-ran full green gate (client+server build) on target branch rather than citing prior result · confirmed calendar_alert_fired whitelist fix and CAL-ALERT: handler both intact post-port · MANIFEST Calendar Alerts row extended to cover both branches | GREEN | (pending tag) | WORDS: ~340 (median 540) — trend ↓ (pure delivery session)
