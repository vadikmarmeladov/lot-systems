...
20260830-01    | ENGINEERING | LOT Email v1 — /email to <name> Log slash-command (logTriggers.ts lot-email); resolves recipient via useCohorts() (Cohort Dating/LOT Community); envelope posted through existing useCreateChatMessage() so it delivers via Sync (LOT Chat) with zero new backend route/model/migration | GREEN | (pending tag) | WORDS: 811 (median 449)
