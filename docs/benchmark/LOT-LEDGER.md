...
20260721-01    | ENGINEERING | Basics Tab Month 1 (LOT-FM-001) — OPEN TAB live: new Basics.tsx (23-item ration ledger, doctrine, price line, status line, TERMINAL tokens), new `basics` route, activated pre-existing disabled nav placeholder, wired public view into both app.tsx (authed) and login.tsx (logged-out), /basics added to server KNOWN_CLIENT_ROUTES. Corrected stale MANIFEST BEST-branch claim (beautiful-johnson-56p7ov never existed with this code). Env fix: node_modules absent in sandbox, installed via npm --legacy-peer-deps (yarn registry blocked by proxy policy) | GREEN | 69d9061 | WORDS: 1028 (median 540)
