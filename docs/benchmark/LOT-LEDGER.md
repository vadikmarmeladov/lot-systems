...
20260817-01    | ENGINEERING | LOT Mail built from scratch — /email log trigger (parseEmailTrigger, log text becomes body), mail_messages table+model, POST/GET /api/mail-messages, SSE delivery (new /sync case), merged into Sync feed tagged MAIL, recipient pool scoped to LOT Community (Usership/Onyx/Legacy/RND/Admin, name-resolved). Fixed .gitignore server/ → /server/ (was silently dropping new src/server/ files) | GREEN  | b2c9d270 | WORDS: 859 (median 449)
