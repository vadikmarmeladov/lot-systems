...
20260907-01    | ENGINEERING | Widget health scan (43 widgets) — ROOT CAUSE FIX: MonthlyPulseWidget month-reveal never fired for any user, joinedAt missing from useProfileView()/UserProfile pick-list; also fixed its own /12 display cap (was frozen at "Month 12" past year one) · CONFIRMED live: formatLog() silent-erasure drops quantum_intent_signal/calendar_entry/self_care_* from AI prompts (memory.ts vs dead question-generator.ts) · TimeWidget fully unlogged · no uniform widget-usage analytics (WidgetErrorBoundary is client-only) · duplicate badge keys quarter_drop/elixir_found · zero widget-level code-splitting (~1.3M JS upfront) | GREEN | (pending tag) | WORDS: 1141 (median 929)
