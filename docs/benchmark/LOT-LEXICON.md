...
MAIL:              LOT Mail — log block label; /email to <name> [msg] trigger; delivery result (recipient, cohort match %) rendered under this label  N  20260722
LOT-MAIL                In-app email over the existing DirectMessage + Sync/SSE channel; /email Log trigger; name resolved via CONCAT firstName+lastName ILIKE; ambiguity broken by findCohortMatches (Cohort/Community similarity) scoped to candidates; named in 4 prior session reports as a ship candidate on now-deleted branches before this first actual ship  N  20260722
```
