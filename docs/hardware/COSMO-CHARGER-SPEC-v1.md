...
3. Place your COSMO® Cube mirror-side DOWN on the pad, centered.
...
Note: The Qi receiver coil sits directly behind the mirror-polished back
      (Side A). That face must contact the pad. Placed this way, the
      display (Side B) still faces up — notifications remain visible
      while charging.
```
...
**v1.1 (2026-07-20):** Corrected §5 charging orientation — the v1.0 instructions
described the polished (coil) side facing up, which would place the coil away
from the pad and prevent inductive coupling. The coil lives behind Side A
(mirror-polished back, per COSMO-DEVICE-SPEC-v1.md §4), so Side A must face
down onto the pad; Side B (display) faces up, keeping notifications readable
while the device charges. No hardware changes — documentation fix only.
