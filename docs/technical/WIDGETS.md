...
- **Data Source:** `/api/memory` endpoint for question generation; `/api/memory/answer` for recording answers (writes a `Log` row server-side directly — not via the client `/api/logs` path)
...
- **Connection:** Submits to `/api/system/submit-feedback`; aggregates community sentiment; reads self-assembly state computed by `selfAssembly.ts`; surfaces Usership-gated transmission block after each assembly run; writes QOS signals (`qos_report_generated`) back into the Quantum Intention Engine on report generation
...
Toggle-based connection widgets for the LOT ecosystem of smart devices. Each widget records connect/disconnect signals to the Quantum Intention Engine for pattern analysis. Six devices make up the full mesh: Car, Home, Computer, Phone, Watch, and Robot (`TOTAL_DEVICES = 6` in `QuantumEngineWidgets.tsx`).

### Ecosystem Status

Displays when at least one device is connected. Shows connected node count (`N/6`) and ecosystem narrative describing the coherence level of the device mesh. When all six devices are connected, records an `ecosystem_full_coherence` composite signal.
...
Connect your phone to the LOT quantum engine. Toggle on/off to record mobile connection state.
...
Connect your watch to the LOT quantum engine. Toggle on/off to record wearable connection state.
...
Connect your robot to the LOT quantum engine. Toggle on/off to record robotics-node connection state.
...
_(System Progress Widget is documented above in the first "System & Metrics Widgets" section — this was a duplicate entry, merged 2026-08-09.)_
