...
An inline block in the Time & Environment stack (not a standalone component — lives directly in `System.tsx`), showing today's ambient conditions: Western sun sign, hourly zodiac animal (six-day 六曜/rokuyo cycle with an auspicious/neutral/caution/inauspicious tag), and moon phase with illumination %. Ambient conditions, not a personal natal chart. One shared snapshot (`getAstrologySnapshot()`) feeds this block, the Quantum Sign Widget's astrology patch, and every new Log entry's `context` — so all three always agree on what today is.

- **Data Source:** `src/shared/utils/astrology.ts` (pure, deterministic ephemeris calculations — no external API)
- **Persistence:** `localStorage` (`astrology-signal-date`) dedupes the once-daily QIE signal publish
- **Connection:** Publishes an `astrology` signal to the Quantum Intention Engine once per day (`recordAstrologySignal`); `getLogContext()` stamps the same snapshot onto every server-created Log's `context`, enabling correlation between calendar rhythm and actual logged activity (Pattern 119, "Auspicious Day Engagement")
