...
**Rev 2 delta (same-day S-2 review):**
```
1. Monthly Archive reveal — was: rotating view, peekable any time mid-month.
                             now: gated. Reveals once, only at tenure-month close.
2. Year One Story        — was: single chronological narrative, capstone-only.
                             now: domain-compressed deliverable (Journaling,
                             Self-Care, Telemedicine, Parenting, Business) and
                             named explicitly as the sales narrative for
                             $1,188/year (12 × $99) Usership.
```
...
**Rev 2 adds the commercial half of the same idea.** Density and badges are the
private, felt evolution. The Year One Story (Section 4.1) is the same
compression mechanism turned into something an operator can point to and say
"this is what my $1,188 bought" — the monthly Archive entries are its raw
material, released one earned month at a time, never browsable ahead of
...
### 3.2 — Monthly Archive, folded into `MemoryWidget` (not a new surface, not a peek)

**Rev 2 correction:** the first pass of this brief described the Archive as one of
`MemoryWidget`'s ordinary rotating views — cyclable any time via Order 7's
click-to-advance. S-2 flagged this correctly: a view the operator can click to any
day mid-month is a *browsable tab*, and a browsable tab is checkable, refreshable,
the thing you go peek at on day 12 hoping it changed. That is streak-shaped
behavior — exactly what Clause 9 forbids — even though no streak counter is on
screen.

The corrected design: the Archive view **does not exist in the rotation at all
until the tenure-month closes.** For 29 (or 30/31) days it is simply absent —
`MemoryWidget` cycles through its normal views only. On the day the tenure-month
boundary is crossed, the Archive view is inserted, once, as the paragraph for
*that* month — generated by `generateMemoryStory()` scoped to the closing 30-day
window instead of all-time. The operator encounters it the next time they open
`MemoryWidget` in the ordinary course of using it — not via a push, not via a
badge on a tab, per Clause 6. It stays available (the operator can always look
back at prior months' entries in an Archive log/history, satisfying the "archive
is the record" clause), but the *reveal* — the moment it first appears in the
rotation — happens exactly once, at month-close, never before.

```
DAY 1–29 of tenure-month:   Archive view absent from MemoryWidget rotation.
DAY 30 (month boundary):    J34 fires generateMonthlyStory() server-side.
NEXT MemoryWidget open:     Archive view enters rotation, one-time reveal,
                            reads "ARCHIVE: MONTH N" — then remains browsable
                            as history from that point on, same as any other
                            past month's entry once it has been earned.
```

This is a stricter reading of Ambient AI than the Rev 1 draft, and the right one:
the system does not narrate progress while it is happening. It reports only when
the period is actually over. That is the difference between a monthly newsletter
(arrives regardless, invites checking) and an archive entry (exists because the
month is now closed, and could not have existed a day earlier).
...
║   ║             ║           ║ hardware"). A domain-       ║         ║                    ║
║   ║             ║           ║ compressed "Year One Story" ║         ║                    ║
║   ║             ║           ║ is compiled (see 4.1) —     ║         ║                    ║
║   ║             ║           ║ the $1,188/year sales        ║         ║                    ║
║   ║             ║           ║ artifact. The                ║         ║                    ║
...
### 4.1 — The Year One Story: domain-compressed, and the sales artifact of Usership

**Rev 2 reframing.** S-2's note: this is not just a nice capstone — it is the
concrete answer to "what do I get for $1,188/year?" (12 × $99/month; no new
billing SKU proposed here, see Positioning below). The Year One Story has to be
good enough, specific enough, and *usable* enough that a prospective operator
sees it and understands they are commissioning a document about their own life,
not renting software.

**Restructure: by domain, not by chronology.** Rev 1 described one linear
narrative arc across twelve months. Rev 2 replaces that with **parallel
domain chapters**, each independently compressed from the year's signal, each in
the same third-person `generateMemoryStory()` voice (intro sentence, blank line,
"Key insights... include:", en-dash bullets). A chronological arc sentence still
closes the document (see below) — but the body is organized the way a person
actually thinks about their year: by area of life, not by calendar order.

```
╔══════════════════════════════════════════════════════════════════════╗
║ DOMAIN         SOURCE SIGNAL              STATUS                     ║
╠══════════════════════════════════════════════════════════════════════╣
║ Journaling     journal (existing source)  Direct — no new capability. ║
║ Self-Care      selfcare (existing source) Direct — no new capability. ║
║ Telemedicine   medical (existing source)  Direct signal, but PRIVATE  ║
║                                            ONLY. Medical cohort is    ║
║                                            already "not surfaced      ║
║                                            publicly" (LOT-WIKI-v72    ║
║                                            §7). This chapter must     ║
║                                            never appear on the public ║
║                                            /u/<username> profile —    ║
║                                            Year One Story delivery is ║
║                                            in-app/export only, always.║
║ Parenting      NOT an existing signal      Requires a new lightweight ║
║                source.                     domain classifier over    ║
║                                            journal/log text (same     ║
║                                            shape as the Word Turn     ║
║                                            engine's keyword detection,║
║                                            repurposed for topic-      ║
║                                            tagging instead of badge-  ║
║                                            triggering).               ║
║ Business       NOT an existing signal      Same as Parenting — new    ║
║                source. Partial overlap     domain classifier, with    ║
║                with planner/goals.         planner/goals as a partial ║
║                                            existing signal to seed it.║
╚══════════════════════════════════════════════════════════════════════╝
```

Honest engineering note (LOT-DOCTRINE, "produce the conditions, don't fabricate
the philosophy" — same principle applied to product copy): **a domain chapter is
included only if the operator actually has signal in it.** An operator with no
medical logs gets no Telemedicine chapter — not a thin, invented one. This keeps
the Year One Story a true compression of a real year, not a template with blanks
filled by flattery. Two of five domains (Journaling, Self-Care) are guaranteed
to populate for any active Usership operator; Telemedicine, Parenting, and
Business populate only where the signal exists — which is itself informative
("this was a journaling-and-self-care year" is a real, honest finding).

**Closing arc sentence.** After the domain chapters, one short paragraph still
carries the chronological thread Rev 1 specified — the **archetype journey**
across the 36 physiological archetypes (Month 3 lock-in → Month 6 cohort →
Month 12 stable signature), e.g. (illustrative, not final copy): *"Their
signature moved from Signal Seeker toward Coherence Builder across the year —
the shift concentrated in months seven through nine."* This is the one place the
Year One Story is allowed to name a trajectory instead of a domain — it is the
thread that ties the five chapters back into one person's arc.

**Positioning — this is the selling point, say it plainly.** Twelve months of
Usership at $99/month is $1,188. The Year One Story is what that number buys, made
tangible: a private, multi-domain, AI-compressed account of the operator's year
that could not have been written by anyone else, because no one else has the
signal. This is the same instinct as `LOT-AI-PRODUCT-BRIEF.md`'s Weekly
Story-Report, scaled from a week to a year and given domain structure instead of
staying purely chronological. Sales copy (external, marketing surface — not
in-app, which stays in COCKPIT-RULE register) can say this directly: *"A year of
Usership is $1,188. At the end of it, you own a Story — journaling, self-care,
health, family, work — written about no one else's year but yours."* In-app, the
artifact itself still never uses that sales register (Order 8 applies to the
product; it does not apply to the storefront).

**Delivery.** Exportable, same shape as the existing Story API sketch in
`LOT-AI-PRODUCT-BRIEF.md` (`POST /api/story/:week_id/export`) — a Year One Story
export target (`{ target: "pdf" | "robot" | "vehicle" | "dashboard" }`) is the
natural extension once the weekly version ships. Domain chapters make a PDF
export meaningfully better than a wall of text: each domain is a page or section,
which is what makes it feel like *owning a document* rather than reading a long
paragraph.
...
NEW CLIENT
  MonthsUnlockedWidget.tsx            new, small, tertiary-opacity, 2-3 cycling
                                       views per Order 7.
  MemoryWidget.tsx                    Archive view is CONDITIONALLY present in
                                       the view rotation, not always-on: absent
                                       until the tenure-month boundary fires,
                                       inserted once server-side flag flips
                                       (monthlyArchiveReady[tenureMonth] on the
                                       user object), never removable/re-hideable
                                       by the operator once revealed — it just
                                       stops being "new."
...
NEW SERVER — DOMAIN COMPRESSION (Year One Story, Section 4.1)
  generateYearOneStory(user, logs)    memory.ts — sibling to generateMemoryStory,
                                       full 12-month scope, returns domain-
                                       keyed sections instead of one paragraph.
  classifyDomain(logText)             new lightweight classifier for Parenting/
                                       Business (no existing signal source
                                       covers them); Journaling/Self-Care/
                                       Telemedicine map directly to existing
                                       journal/selfcare/medical sources — no
                                       classifier needed for those three.
  Telemedicine privacy gate           Year One Story's medical/Telemedicine
                                       chapter must inherit the existing medical-
                                       cohort privacy rule ("not surfaced
                                       publicly," LOT-WIKI-v72 §7) — in-app and
                                       export only, never on /u/<username>.
  POST /api/story/year-one/export     new export target, same shape as the
    { target: "pdf" | ... }            Weekly Story-Report export sketch in
                                       LOT-AI-PRODUCT-BRIEF.md.
...
YEAR ONE STORY — DOMAIN CHAPTER (same voice, one chapter of several; a domain
with no signal is simply omitted, never filled with a generic placeholder):
  "SELF-CARE

  This operator's self-care activity concentrated in the evening across the
  year, with a marked increase in the second half.

  Key insights into their self-care patterns include:
  – Self-care logging rose from 8 entries in months 1-6 to 22 in months 7-12
  – Evening (20:00-22:00) was the dominant window in 9 of 12 months
  – Recovery-window signal followed depleted-energy days in the majority
    of instances after month 6"

SALES COPY (external/marketing register only — never in-app):
  "A year of Usership is $1,188. At the end of it, you own a Story —
  journaling, self-care, health, family, work — written about no one
  else's year but yours."
```
...
RISK 6   Archive reveal timing feels arbitrary if the operator opens MemoryWidget
         at 00:01 on month-close vs. three days later. Mitigation: the reveal is
         defined by J34's daily tenure-check (00:00 UTC) writing the readiness
         flag, not by when the operator happens to look — "earned, not pushed"
         means the operator may not notice for days, and that is correct, not a
         bug to fix with a notification.

RISK 7   Telemedicine chapter privacy. The Year One Story is the first artifact
         in this brief that could plausibly surface medical signal in a document
         an operator might export or share. Mitigation is non-negotiable: inherit
         the existing "not surfaced publicly" medical-cohort rule outright — the
         Telemedicine chapter exists in-app and in direct export only, is never
         included in any profile-adjacent or shareable-link surface, and should
         default to an explicit operator opt-in before appearing in an exported
         PDF at all (stricter than the other four domains, which can default in).
```
...
Month 12  instrument    full 43 widgets   YEAR ONE STORY        year_one ≋∞
                                          domain-compressed: Journaling,
                                          Self-Care, Telemedicine (private-
                                          only), Parenting, Business —
                                          the $1,188/year sales artifact
