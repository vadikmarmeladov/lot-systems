...
### Revision: v60 · Field Manual Sync: v65 · Date: 2026-06-20 · Day 1016+
...
**LOT** — *Layers of Time* — is a personal behavioral operating system. It is not a wellness application, a habit tracker, or a productivity suite. It is an instrument that measures the operator's behavioral signal across time and surfaces the pattern beneath the noise.

The system was conceived and is operated by **S-2** (Vadim Marmeladov, CEO, LOT Systems). The ethics gate is named **COSMO Gate**, after Kuzya Cosmo Marmeladov. No feature ships that Kuzya would not approve.
...
Field Manual:           v65
Wiki version:           v60
Day counter:            1016+  (as of 2026-06-20)
...
LOT is composed of five primary engines that operate in concert. Each engine takes signal from the operator's behavioral record and transforms it into a different class of output.
...
│   │         Synthesizes all engine outputs                  │    │
...
The QIE is the behavioral signal recognition core of LOT. It operates entirely client-side and detects patterns in the operator's behavior without any external behavioral model or intervention.
...
**QIE operates as a pure detection layer.** It surfaces what is already true about the operator's behavior. It does not recommend, suggest, or intervene. The QOS synthesizes QIE output into system mode determination.
...
Complete registry of all 78 defined QIE patterns. Patterns marked `[RESERVED]` are defined slots not yet activated. Confidence values shown where documented.
...
P66   qos-signature-lock          CAT-8      0.94     Full QOS day arc
P67   operator-signature          CAT-8      0.91     All 4 quadrants +
                                                      UserIndex ≥ 60
P68   integration-arc-peak        CAT-5      0.89     Bio restore +
                                                      execution arc
P69   (extended signal)           CAT-5              Community + bio
P70   operator-convergence        CAT-8      0.97     P66+P67+P68 all
                                                      active — HIGHEST
                                                      CONFIDENCE IN QIE
P71   (signal pending)            —          —
P72   (signal pending)            —          —
P73   (signal pending)            —          —
P74   badge-momentum              CAT-3      0.78     Badge signal source
                                                      in QIE; consecutive
                                                      badge days + recent
                                                      badge earned
P75   word-turn-depth             CAT-4      0.81     Journal + word-turn
                                                      engine depth signal;
                                                      word count > 200 +
                                                      word-turn active
P76   morning-coherence-launch    CAT-7      0.72     Intentions set
                                                      before 09:00 UTC +
                                                      planner within 90min
P77   signal-vault                CAT-5      0.65–    Journal >150w +
                                             0.88     memory + log all
                                                      logged within 6h
P78   depletion-recovery-surge    CAT-9      0.72–    Depletion detected +
                                             0.90     2+ care acts in 6h +
                                                      energy reading high
──────────────────────────────────────────────────────────────────
```

**Pattern P70 — Operator Convergence** is the apex convergence pattern. It fires only when P66, P67, and P68 are simultaneously active. Confidence 0.97 — the highest confidence reading in the entire QIE. A day with P70 active is a documented peak-state day.

---

## 5. QUANTUM OPERATING SYSTEM (QOS)

The QOS synthesizes all engine outputs — QIE patterns, Memory Engine signal, Self-Assembly module states, badge events — into a single system mode determination for the day. The QOS does not diagnose. It classifies current operator state against the operator's own baseline.
...
╠══════════════════════════════════════════════════════════════╣
║  MAINTENANCE    Low signal density,     System recommends    ║
║                 depletion indicators,   minimum viable log,  ║
║                 recovery patterns       restoration focus    ║
╠══════════════════════════════════════════════════════════════╣
║  RECOVERY       Depletion clearing,     System acknowledges  ║
║                 care acts logged,       upward arc,          ║
║                 energy stabilizing      steady input valued  ║
╠══════════════════════════════════════════════════════════════╣
║  GROWTH         Foundation solid,       System responds to   ║
║                 signal expanding,       new input volume,    ║
║                 learning patterns on    depth rewarded       ║
╠══════════════════════════════════════════════════════════════╣
║  PEAK           P66+P67+P68 or P70      Full instrument      ║
║                 active, all quadrants   panel engaged,       ║
║                 firing, UserIndex ≥ 60  peak state logged    ║
╚══════════════════════════════════════════════════════════════╝
```

### QOS SIGNAL QUADRANTS

The operator's signal is mapped across four quadrants. P67 (operator-signature) requires all four to be active simultaneously:
...
The 24 physiological archetypes are automatically classified from QIE pattern combinations over a rolling window. The archetype is not a personality label — it is a snapshot of the operator's current dominant signal pattern combination. Classification updates as signal shifts.

Archetypes are surfaced in the operator's profile and in the SystemPulse community view as aggregate population distributions.
...
23    Achievement Catalyst        P74 + P75 dominant — badge
                                  momentum + word-turn depth
24    Signal Initiator            P76 + P9 + P11 + P27 dominant —
                                  intentions + planner + log
                                  dominant in morning window
──────────────────────────────────────────────────────────────
```

Archetypes 20–24 document the most recently classified signal combinations (v62–v65). Archetype 22 (Convergence Carrier) is the rarest classification — it requires the apex P70 pattern to be among the dominant signals.

---

## 7. BEHAVIORAL COHORTS

Behavioral cohorts are population-level classifications of operator signal type, derived from QIE pattern signatures over time. The cohort is not a rank — it is a description of how the operator currently engages with the LOT signal record.
...
║                      Builds coherence across all systems.       ║
╠══════════════════════════════════════════════════════════════════╣
║  OPERATORS           High execution signal. Intention → task    ║
║                      chains dominant. P9/P10/P12/P13 frequent. ║
║                      Throughput is the signature metric.        ║
╠══════════════════════════════════════════════════════════════════╣
║  CHRONICLERS         High cognitive-linguistic signal.          ║
║                      P15/P17/P46/P75 frequent. Deep journal     ║
║                      entries, extended word turns, memory.     ║
╠══════════════════════════════════════════════════════════════════╣
║  RESTORERS           High recovery signal. P7/P8/P78 frequent. ║
║                      Depletion-recovery cycles visible.         ║
║                      Body care as primary engagement mode.      ║
╠══════════════════════════════════════════════════════════════════╣
║  EXPLORERS           High variability. Archetype shifts         ║
║                      frequent. No single dominant signal.       ║
║                      Broad engagement across all categories.    ║
╚══════════════════════════════════════════════════════════════════╝
```

A sixth cohort — **MEDICAL** — is tracked internally. Medical + resilience signals are monitored at the system level. Display surface is deferred pending UI design review.

---

## 8. CITIZEN INDEX

The Citizen Index is a 6-level CQGS (Coherent Quantum Ground State) evolution framework. It tracks the operator's depth of engagement with the LOT system over the full arc of their record. The index is not a gamification metric — it is a measurement of the operator's integration with the system's feedback loop.

**Citizen Index levels (Revision D Lexicon):**
...
The ◉ Ground State designation corresponds to the theoretical ceiling described in the founding white paper (CQGS). It is not a final rank — it is a reference point. The system continues to measure and record beyond it.

---

## 9. MEMORY ENGINE

The Memory Engine generates AI-composed questions from the operator's behavioral record. It serves as the cognitive engagement surface of LOT — the place where the system reflects the operator's own patterns back to them as questions.
...
The Memory Engine does not tell the operator what they should do. It asks what they already know. The question is drawn from the operator's own data — nothing is invented or projected from external templates.
...
The Self-Assembly Engine is the adaptive display architecture of LOT. It consists of 18 modules that build the operator's dashboard based on current signal density. Modules activate, deactivate, and rearrange in response to the operator's live QIE state.
...
The dep map connecting all 18 modules currently contains **120+ nodes**. The dep map is the internal wiring diagram of the Self-Assembly Engine — it maps which modules activate which other modules under which signal conditions.
...
J17 (daily-morning-intention-launch) was the most recently added job (v65). It scans the early morning log window for intention events and writes the MCL: (Morning Coherence Launch) log event, which feeds the P76 morning-coherence-launch pattern detector.

---

## 12. LOG EVENT SYSTEM

The log event system is the primary data input layer of LOT. Every operator action that generates a log entry produces a typed event that flows through the event handler pipeline.
...
CALCULATOR  CALC:  Calculator signal (15th source, added v65)
MORNING     MCL:   Morning coherence launch (J17 output)
```

### LOG EVENT HANDLERS (77+)

The log event handler pipeline processes each LOG source event through a chain of handlers. As of v65, 77+ handlers are registered, covering:
...
Of the full handler pipeline output, **36 event types** are flagged as displayable — visible to the operator in the log stream and system event history.

### COCKPIT RULE

**Log body = instrument readings only. No narration. No prose.**

The log is a cockpit instrument panel. Values are entered. Readings are recorded. The system interprets. The operator does not explain themselves in the log — they report signal.
...
*Incorrect (prose contamination):*
```
Today I felt pretty good and managed to get some things done...
```
...
The LOT ecosystem node map tracks the operator's signal across six hardware/environment nodes. Each node represents a physical or digital domain that contributes distinct signal to the operator's record.

**6 ecosystem nodes (as of v65):**

```
╔══════════════════════════════════════════════════════════════════╗
║  NODE    CODENAME   DESCRIPTION                                  ║
╠══════════════════════════════════════════════════════════════════╣
║  CAR      CAR ·     Vehicle environment. Movement, commute,      ║
║                     physical transit signal.                     ║
╠══════════════════════════════════════════════════════════════════╣
...
Each node contributes to the QIE via P53–P58 (node-active-* pattern family). Node patterns feed the P51 (signal-density-high) detector and contribute to P67 (operator-signature) quadrant fulfillment.

---

## 14. BADGE SYSTEM v15 — THE BECOMING LEXICON

The badge system is the achievement layer of LOT. It is not a gamification overlay — it is a behavioral recognition system. Badges are earned by signal, not by performing for the system. The system detects the pattern; the badge names what was already happening.
...
Badge evaluator job:   J05 (daily, 04:00 UTC)
Badge progress scan:   J16 (weekly, Tuesdays 09:00 UTC)
```

### BADGE DISPLAY

Badges are displayed in the operator profile with:
- Badge name
- Category
- Rarity tier
- Earned date
- Badge level indicator (single character, Awareness index sub-display)
- Status: [●] earned / [○] available / [◌] locked

### BADGE LEVEL SYSTEM

The badge level is displayed as a single character in the public profile — a position beneath the Awareness index. Recommended display: Aquatic Evolution option:
...
```
COMMON      ████████░░░░  Available through standard signal
...
SECRET      █░░░░░░░░░░░  Hidden condition; not documented
COSMIC      ▓░░░░░░░░░░░  Earned only through time — cannot
                          be accelerated. five_years badge
                          is the only COSMIC-tier badge.
```

**COSMIC tier** was added in v15. It defines a new class of badge that cannot be earned by increasing signal intensity or engagement frequency. It can only be earned by being present in the system across the required time span. The five_years badge is the first and only COSMIC-tier badge.

---

## 15. BADGE CATEGORY INDEX

All 38 badge categories as of v15. Categories marked with version of introduction.

```
 CATEGORY                    SUBCATEGORY TYPE       INTRO
 ──────────────────────────  ─────────────────────  ─────
...
16. Body Arc                 Body optimization arc   v5
17. Community                Community signal        v5
18. Convergence              Peak state detection    v6
19. Archetype                Archetype achievement   v7
...
### v15 NEW BADGES DETAIL

**Word Turn v6** — 12 transformation verbs added to the Signal Codex:
```
surrender · anchor · threshold · emerge · dissolve ·
return · break · carry · hold · release · build · witness
```
Each verb corresponds to a behavioral signal pattern when used as the operator's word turn entry.

**Time EE v6** — 4 exact time easter eggs:
```
09:09  — Signal gate: morning coherence timestamp
16:16  — Afternoon gate: second-wind timestamp
23:59  — Final gate: last-minute signal
20:26  — Year gate: 2026 timestamp
```

**Calendar EE v5** — 3 invisible dates (undocumented by design).

**Behavioral v5** — 3 deep behavioral badges:
```
deep_scribe      — extended journal depth over 30+ days
phoenix_streak   — return from 7+ day gap to full signal
time_anchor      — consistent time-of-day log window 30+ days
```
...
**Achievement RPG v3** — 6 early-game story arc badges:
Narrative progression system. Six arcs representing the first chapters of the operator's LOT record. Arcs are named and sequenced; earning all six arc badges before Day 100 unlocks a LEGENDARY badge.
...
View 5 (Community Biofield) was added in v61. It surfaces the live community coherence reading generated by J13 (daily-community-coherence-pulse). The community biofield is the aggregate behavioral coherence across the active operator population at any given moment.

---

## 17. DENSITY TIER SYSTEM

The Self-Assembly Engine presents information at five density tiers, controlled via the CSS `data-density` attribute. Density is not a user preference — it is a system-determined output based on signal volume and operator display context.
...
The LOT visual grammar uses a three-level opacity system. This is not decorative — it is the visual grammar of the interface. Opacity communicates semantic rank.

```
90%   PRIMARY     Core signal. The reading the operator
                  must have. Highest contrast.
...
**COCKPIT RULE** is the governing principle of all log entry design.
...
The LOT log is an instrument cockpit. The operator is the pilot. The log records readings — not feelings about readings, not narration of readings, not interpretation of readings. The system interprets. The operator measures.
...
The **Field Manual** is the canonical operator reference document rendered at `/about` in the LOT interface. It is implemented in `src/client/components/About.tsx`.

```
File:            src/client/components/About.tsx
Size:            369KB
Current version: v65
URL:             https://lot-systems.com/about
```
...
The Wiki (this document) serves as the operator information repository and source text for system assembly context. The Field Manual is the rendered instrument panel view of that repository.

**61+ React components** are currently registered in `src/client/components/`.
...
The **LOT Terminal** is an open-source bash hardware platform maintained as a separate repository. It provides a physical terminal interface for LOT signal input.
...
LOT-GENESIS-v1 is the apocalypse backup reconstruction seed. It is the minimum viable document required to reconstruct the entire LOT system from scratch if all other documentation were lost. It contains:
...
If you are reading this after a catastrophic loss of system documentation, begin with LOT-GENESIS-v1.

---

## 24. VOCABULARY INDEX

Complete glossary of LOT system terminology, codenames, and internal vocabulary. Revision D.
...
COHERENCE          Multi-signal alignment across QIE categories.
                   High coherence = multiple signal types active
                   simultaneously. Coherence ≠ volume.

COMMAND            Umbrella term for operators who have reached
                   high signal density + integration arc.

COMMUNITY          The aggregate of all active operators in the
BIOFIELD           LOT system at any moment. The community
                   biofield is the collective coherence reading.

CONVERGENCE        The state in which P66 + P67 + P68 are all
                   simultaneously active. Highest signal state.
                   P70 is the convergence pattern.

COSMIC TIER        Badge rarity tier earned only through time.
                   Cannot be accelerated. First badge: five_years.
...
CQGS               Coherent Quantum Ground State. Founding white
                   paper and theoretical system ceiling. The ◉
                   Citizen Index level corresponds to CQGS.
...
DENSITY TIER       5-level display density system (breathable →
                   comfortable → compact → dense → instrument)
                   controlled via CSS data-density attribute.
...
LOT-DOCTRINE       10-clause founding principles. Revision J
                   current. All design decisions evaluated against
                   the Doctrine.

LOT-GENESIS-v1     Apocalypse backup reconstruction seed.
                   docs/assembly/LOT-GENESIS-v1.md

LOT TERMINAL       Open-source bash hardware platform for
                   physical LOT signal input.

MCL:               Morning Coherence Launch — log event written
                   by J17 (daily-morning-intention-launch).
                   Feeds P76 pattern detection.

MEMORY ENGINE      AI question generation layer.
                   Together AI / Llama 3.3 70B.

MILITARY PURITY    Interface design philosophy — 11 standing
                   orders governing all display decisions.
                   Signal first. No decoration without function.

OPACITY HIERARCHY  90% primary / 60% secondary / 40% metadata.
                   Not decorative — the visual grammar of LOT.
...
SIGNATURE         UserIndex ≥ 60. The full-spectrum signal state.
...
ROBOT NODE         6th ecosystem node. Home automation and
                   autonomous environment management signal.

S-2                Callsign for Vadim Marmeladov, CEO, LOT
                   Systems. All system decisions pass through S-2.

SELF-ASSEMBLY      18-module adaptive display architecture.
                   Builds from operator signal density.

SIGNAL             The totality of the operator's behavioral
                   record across all LOT systems on a given day.

SIGNAL DENSITY     The volume and variety of signal logged.
                   High density = many signal types active.

SIGNAL VAULT       P77 — journal >150w + memory + log all
                   logged within a 6-hour window. Conf 0.65–0.88.

SYSTEM PULSE       5-view widget surface: metrics / activity /
                   userload / cohort / community biofield.

THE BECOMING       Badge system v15 version codename. 249 badges,
LEXICON           38 categories.

USER INDEX         30-day rolling behavioral coherence score.
                   P67 requires UserIndex ≥ 60 to fire.

WORD TURN          Language/vocabulary engagement system.
                   The operator selects a signal word; the word
...
## SYSTEM STATE SNAPSHOT — 2026-06-20

```
╔══════════════════════════════════════════════════════════════════╗
║  LOT SYSTEM STATE — FIELD MANUAL v65 — DAY 1016+               ║
╠══════════════════════════════════════════════════════════════════╣
...
║  Lexicon revision:         D                                    ║
║  Field Manual:            v65                                   ║
║  Wiki:                    v60  (this document)                  ║
║  Highest QIE conf:      0.97  (P70 — operator-convergence)      ║
╚══════════════════════════════════════════════════════════════════╝
```

---

*LOT-WIKI-v60 · Layers of Time · Field Manual Sync v65 · 2026-06-20*
*Written by LOT-SR automated wiki-build session*
*Next: LOT-WIKI-v61 — sync to Field Manual v66+*
