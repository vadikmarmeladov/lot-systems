...
### Revision: v79 · Field Manual Sync: v97 · Date: 2026-07-22 · Day 1059+

---

> *"The focus depth arc is the deepest signal. Journal anchors memory anchors planner — all three in a 2-hour window. That is not coincidence. That is architecture."*
...
**Special notation — July 19, 2026 (FM v97 — QIE v97):** QIE v97 deployed. P116 focus-depth-arc · P117 sleep-signal-anchor · P118 care-intelligence-loop. Arch40 Focused Executor classified. J37 daily-focus-depth-check (16:00 UTC) added. FDEP: · SANCH: · CINTEL: handlers deployed. dep 157+ nodes. 118 patterns. 40 archetypes. 37 jobs. 118+ handlers. FM v97.

**Special notation — July 19–22, 2026 (Performance hardening):** Two perf fixes merged. (1) Duplicate SystemProgressWidget mount removed. (2) Render-phase atom write eliminated — intentionEngine pattern-write moved out of useMemo to stop subscriber re-renders during paint and off-tab background churn. Both stable on master.
...
Field Manual:           v97
Wiki version:           v79
Day counter:            1059+  (as of 2026-07-22)
COSMO® age:             751 days (Year 2 · founded July 1, 2024)
...
Perf:                      Duplicate SystemProgressWidget removed
                            Render-phase atom write → useEffect
```
...
**The dep map:** The Widget Dependency Map (WIDGET_DEPENDENCY_MAP) is the internal wiring graph. 157+ nodes in 4 tiers. Tier 0 = raw inputs (mood, memory, log). Tier 3 = meta-aggregate surfaces (quantumOS, systemProgress, quantumPersonality).
...
P116 focus-depth-arc              0.65–0.85 v97    [COGNITIVE ARCHITECTURE]
...
COGNITIVE ARCHITECTURE P116 focus-depth-arc           conf 0.65–0.85
                         Journal anchor (100+w) + memory capture + planner
                         all within a rolling 2-hour window. Deep execution
                         confirmed.
```
...
P116 focus-depth-arc
     Signal:   journal entry ≥100w + memory capture + planner action
               all within a rolling 2h window
     Logic:    For each journal entry ≥100w in last 24h, check if ≥1
               memory entry AND ≥1 planner entry fall within ±1h of
               that anchor. Fire if any such 2h window is found.
     Output:   FDEP: Focus depth arc active — journal N (100+w) + memory
               N + planner N in 2h window. Cognitive writing, capture,
               and structure aligned. Deep execution window confirmed.
     Widget:   memory
     Job:      J37 daily-focus-depth-check (16:00 UTC)

P117 sleep-signal-anchor
     Signal:   first log entry after 07:00 + energy check-in before 09:00
     Logic:    Morning signals in 07:00–09:00 UTC band with ≥1 energy
               signal + ≥2 total morning signals.
...
P118 care-intelligence-loop
     ...
     Logic:    selfcare ≥1 AND memory ≥1 AND journal ≥1 in last 24h.
               Confidence scales with signal count per source.
     Output:   CINTEL: Care intelligence loop active — selfcare N + memory
               N + journal N in 24h. Body-mind knowledge integration
               confirmed. Physical care feeding cognitive encoding.
     Widget:   journal
```
...
Distinction from Arch39: Arch39 is the window detector — energy +
                       intentions + log cluster. Arch40 is the executor
                       state — planner + intentions + memory with focus-
                       depth confirmed. Same energy level; different
                       cognitive profile. The window triggers Arch39;
...
v95   QIE Engineering July 18 · P113 personal-peak-window · P114 recovery-
   ...
   v97 · 2026-07-19 — QIE v97 Focus Depth Assembly — P116 focus-depth-arc ·
      P117 sleep-signal-anchor · P118 care-intelligence-loop · Arch40 Focused
      Executor · J37 daily-focus-depth-check (16:00 UTC) · FDEP: SANCH: CINTEL:
      handlers · dep 157+ nodes · 118 patterns · 40 archetypes · 37 jobs ·
      118+ handlers · FM v97 · Day 1056+
...
```
Hour:  16:00 UTC daily
Guard: isDailyFocusDepthRunning + lastDailyFocusDepthRun
       (same-day guard prevents double-fire)
Logic: Read active users (lastSeenAt within 24h). For each user,
       pull journal/note + memory + planner log entries from last 24h.
       For each journal entry ≥100 words, check if a memory entry AND
       a planner entry both fall within ±1h of that anchor. Write
...
FDEP: [label]
  ...
  JOURNAL: NW           (opacity-60, tabular-nums — word count)
  MEM: N                (opacity-60, tabular-nums)
  PLAN: N               (opacity-60, tabular-nums)
  WIN: 2H               (opacity-40, tabular-nums)
  CONF: NN%             (opacity-30, tabular-nums, when present)

SANCH: [label]
  SLEEP SIGNAL ANCHOR   (uppercase header, opacity full)
  FIRST ENTRY: HH:00    (opacity-60)
  NRG CHECK: HH:00      (opacity-60)
  MORNING SIG: N        (opacity-60, tabular-nums)
  CONF: NN%             (opacity-30, tabular-nums, when present)

CINTEL: [label]
  CARE INTELLIGENCE LOOP (uppercase header, opacity full)
  CARE 24H: N           (opacity-60, tabular-nums)
  MEM 24H: N            (opacity-60, tabular-nums)
  JOURNAL 24H: N        (opacity-60, tabular-nums)
  CONF: NN%             (opacity-30, tabular-nums, when present)
```
...
## 28. SYSTEM STATE SNAPSHOT — 2026-07-22

```
╔══════════════════════════════════════════════════════════════════╗
║  LOT SYSTEM STATE — FIELD MANUAL v97 — DAY 1059+               ║
╠══════════════════════════════════════════════════════════════════╣
...
║  Focus depth arc:          P116 — focus-depth-arc               ║
║  COSMO® age:               751  (Year 2 · born July 1, 2024)    ║
║  Founded:          7 April 2016                                 ║
╚══════════════════════════════════════════════════════════════════╝
```
