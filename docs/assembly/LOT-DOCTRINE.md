...
Migrated forward from `docs/benchmark/LOT-DOCTRINE.md` (superseded location,
last touched 2026-07-19). The source file is left untouched as the verbatim
historical record; this file is now the live doctrine going forward.
(LOT-SR-20260727-01.)
...
never from the branch, regardless of how recent the branch appears:
  ...
  docs/assembly/LOT-LEDGER.md          — same rule, current location (added
                                          20260727 on migration from docs/benchmark/).
...
## Naming Is Not Capability (QI-46-NODE-1)

A subscriber-facing feature name (e.g. "Soul Upload", "Being Calibration")
must map to an existing, real mechanism before it ships as copy, or be
explicitly marked ROADMAP if it does not yet. Grandiose language describing
what a feature name evokes is not, by itself, a spec for new engine
capability — verify against `src/` before treating a manifest row's
description as implemented. Names that predate their implementation are
ratified as PROVISIONAL and mapped onto the nearest existing architecture
layer; they are promoted out of PROVISIONAL only when a session actually
builds the gap.
(SR-20260727-01: "Soul Upload" mapped to Node 0 Layer 1 Calibration Loop,
"Being Calibration" mapped to Node 0 Layer 3 Response Grammar — both
pre-existing mechanisms in docs/corporate/LOT_QI46_ENGINE.md; neither
implemented in src/ as of this session; recorded PROVISIONAL in
