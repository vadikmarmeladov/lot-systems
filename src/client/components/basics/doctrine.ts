...
// Entitlement (the Basic tag) is granted by S-2 after billing is confirmed
// externally, same as USERSHIP. Enrollment here only submits roster intake.

export const BASICS_SIZES: { value: string; label: string }[] = [
  { value: 'S', label: 'S' },
  { value: 'M', label: 'M' },
  { value: 'L', label: 'L' },
  { value: 'XL', label: 'XL' },
...
export const SHIPPING_FIELDS: {
  key: 'name' | 'address1' | 'address2' | 'city' | 'region' | 'postalCode' | 'country'
  label: string
  required: boolean
}[] = [
  ...
  { key: 'region', label: 'REGION / STATE', required: true },
