...
export const System = React.memo(function SystemInner() {
  ...
  // timeZone when known (matches the server-side Logs snapshot in
  // getLogContext), falling back to device-local time otherwise — so the
  // reading is personalized to the user rather than whatever clock the
  // viewing device happens to be set to.
  const astrology = React.useMemo(() => {
    const now = getWallClockDate(new Date(), me?.timeZone)
