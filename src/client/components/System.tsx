...
export const System = React.memo(function SystemInner() {
  ...
  // rokuyo), not a personal natal chart. Reads the operator's saved
  // timeZone (same wall-clock-passthrough trick as the server-side Logs
  // snapshot in getLogContext()) so the reading matches home timeZone, not
  // whatever timeZone the current device happens to be set to. Falls back
  // to device-local time before the profile has loaded.
