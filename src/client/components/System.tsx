...
export const System = React.memo(function SystemInner() {
  ...
  // timeZone when available (same wall-clock-passthrough trick as the
  // server's Logs snapshot, src/server/utils/logs.ts toWallClockDate), so
  // someone viewing the dashboard on a device set to a different timeZone
  // than their profile still sees their own zodiac hour / day boundary —
  // not the device's. Falls back to device-local time when unset.
  const astrology = React.useMemo(() => {
    let now = new Date()
    const timeZone = me?.timeZone
    if (timeZone) {
      try {
        const zoned = dayjs().tz(timeZone)
        now = new Date(zoned.year(), zoned.month(), zoned.date(), zoned.hour(), zoned.minute(), zoned.second())
      } catch {
        // Unrecognized IANA timeZone string — fall back to device-local time above.
      }
    }
