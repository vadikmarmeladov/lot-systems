...
export const System = React.memo(function SystemInner() {
  ...
  // rokuyo), not a personal natal chart. Personalization anchor: read the
  // wall-clock in the user's saved timeZone (same trick as the server's
  // getLogContext) instead of device-local time, so the dashboard reading
  // matches the one already stamped onto Logs entries. Falls back to
  // device-local time when no timeZone is saved yet.
