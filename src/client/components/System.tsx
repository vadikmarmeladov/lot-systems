...
import { getHourlyZodiac, getWesternZodiac, getMoonPhase, getRokuyo, getMoonEmoji, toWallClockDate } from '#shared/utils/astrology'
...
export const System = React.memo(function SystemInner() {
  ...
  // (same wall-clock trick as the server-side Logs snapshot in
  // server/utils/logs.ts) so the dashboard block matches what a user on a
  // device set to a different timeZone than their profile would see in
  // their journal, falling back to device-local time when no profile
  // timeZone is set (e.g. logged out, or never configured in Settings).
