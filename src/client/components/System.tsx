...
import { getHourlyZodiac, getWesternZodiac, getMoonPhase, getRokuyo, getMoonEmoji, toTimeZoneWallClock } from '#shared/utils/astrology'
...
export const System = React.memo(function SystemInner() {
  ...
  // when available (same wall-clock approach as the server-side Logs
  // snapshot in logs.ts) so the reading matches "home", not the viewing
  // device's local clock.
  const astrology = React.useMemo(() => {
    const now = me?.timeZone ? toTimeZoneWallClock(new Date(), me.timeZone) : new Date()
