...
export const StatusPage = ({ noWrapper = false }: StatusPageProps) => {
  ...
  const getStatusIcon = (checkStatus: 'ok' | 'error' | 'unknown') => {
    switch (checkStatus) {
      case 'ok':
        return <span aria-label="Operational">✓</span>
      case 'error':
        return <span aria-label="Error">✕</span>
      case 'unknown':
          ...
          {memoryStatus && (
            <div className="mb-16 pt-32 border-t border-acc/20">
              ...
              <Block label="Next prompt:" labelClassName="!pl-0">
                <div className="flex items-center gap-x-8">
                  <span aria-label={memoryStatus.nextPromptAvailable ? 'Available' : 'Not available'}>{memoryStatus.nextPromptAvailable ? '✓' : '✕'}</span>
                  <span className={cn(
