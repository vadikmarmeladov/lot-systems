...
function statusClass(status: EntryStatus): string {
  if (status === 'EXECUTE') return 'text-red'
  if (status === 'ALERT') return 'text-yellow'
...
export function CalendarWidget() {
  ...
  // Fire an in-widget alert for events within 60 minutes (timed entries only),
  // and — if the operator has granted permission — a real OS-level notification
  // so a due event still reaches them with the tab in the background. Each
  // entry alerts at most once per browser session (deduped via alertedRef).
  React.useEffect(() => {
    const check = () => {
      const n = dayjs()
      for (const entry of entries) {
        if (!entry.time || !entry.id) continue
        const key = entry.id
        ...
        if (diffMin >= 0 && diffMin <= 60) {
          ...
          const status: 'EXECUTE' | 'ALERT' = diffMin <= 15 ? 'EXECUTE' : 'ALERT'
  ...
  const handleAddEntry = () => {
    ...
    createLog({
      text: `${entryType.toUpperCase()}: ${entryText.trim()} (${dateLabel}${timeLabel})`,
      ...
      onSuccess: () => {
        // staleTime on useLogs blocks a plain invalidateQueries from refetching —
        // force it so the new entry appears immediately (see SR history 2026-06-27).
        ...
        <div className="w-full">
          ...
          {todayEntries.length > 0 && (
            <div className="mb-16">
              ...
              {todayEntries.map((e) => {
                ...
                return (
                  <div key={e.id} className="flex items-baseline gap-8 mb-2 text-sm">
                    {e.time
          ...
          <div className="mb-16 flex items-center gap-16">
            ...
            {notifyPermission === 'default' && (
              <button
                className="text-acc/30 hover:text-acc/60 text-xs uppercase tracking-widest transition-opacity"
            ...
            <div className="mb-16">
              ...
              {selectedDate && entriesOnDate.length > 0 && !isAddingEntry && (
                <div className="mt-8">
                  ...
                  {entriesOnDate.map((e) => (
                    <div key={e.id} className="flex gap-8 items-baseline mb-2 text-sm">
          ...
          {upcomingEntries.length > 0 && (
            <div className="space-y-2">
              {upcomingEntries.map((entry) => (
                <div key={entry.id} className="flex gap-8 text-sm items-baseline">
          ...
          <div className="border border-acc/40 bg-bac grid-fill p-16 max-w-64">
            <div className="flex justify-between items-baseline mb-8">
              <span className={cn(
                ...
                activeAlert.status === 'EXECUTE' ? 'text-red' : 'text-yellow'
              )}>
