 ...
 *
 * BASICS — LOT-FM-001. Hardware/physical layer of the LOT System.
 * Month 1 build: OPEN TAB. Read-only ledger — 23-item ration manifest,
 * doctrine, price line, status line. No enrollment logic yet (M2).
 */
...
const LedgerCategory: React.FC<{ category: RationCategory; items: RationItem[] }> = ({
  category,
  items,
}) => (
  <div className="mb-12">
    ...
    >
      ...
      {CATEGORY_ORDER.map((category) => (
        <LedgerCategory
          key={category}
          category={category}
          items={RATION_MANIFEST.filter((item) => item.category === category)}
        />
      ))}
      ...
      <div className="mb-16">
        <StatusLine plan={planLabel} ration={rationStatus} />
      </div>

      {/* UPGRADE PATH — scaffold visible, enrollment gated to M2 build cycle */}
      {!isOnStrength && (
        <div className="mt-4">
