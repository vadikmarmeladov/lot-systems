/**
 * LOT SYSTEMS CORPORATION
 * Vadim Marmeladov — CEO, Owner LOT®
 * Kuzya Cosmo Marmeladov — CEO, Owner COSMO®
 * LOT® Founded 7 April 2016 | COSMO® Founded 1 July 2024
 * Made in the USA | brand.lot-systems.com
 *
 * LOT-FM-001 / BASIC RATION MODULE
 * 3-month build: M1 OPEN TAB (public ledger) · M2 UPGRADE + roster state
 * machine · M3 issue + fulfillment engine.
 */

import * as React from 'react'
import { useStore } from '@nanostores/react'
import * as stores from '#client/stores'
import { cn } from '#client/utils'
import { useDocumentTitle } from '#client/utils/hooks'
import {
  RATION_MANIFEST,
  DOCTRINE_LINES,
  PRICE_LINE,
  MANUAL_REF,
  RATION_COUNT,
  type RationItem,
  type RationCadence,
} from '#shared/constants/basics'
import type { BasicsSize } from '#shared/types'
import {
  useBasicsState,
  useBasicsEnroll,
  useBasicsConfirm,
  useBasicsStandDown,
  useBasicsDispatchIssue,
} from '#client/queries'

// ─── terminal grid primitives ──────────────────────────────────────────────

const HeavyRule: React.FC<{ className?: string }> = ({ className }) => (
  <div className={cn('w-full', className)}>
    <div className="border-t-2 border-acc w-full" />
  </div>
)

const ThinRule: React.FC<{ className?: string }> = ({ className }) => (
  <div className={cn('border-t border-acc/30 w-full', className)} />
)

const SectionLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="font-mono text-[11px] tracking-widest uppercase text-acc/50 py-8">
    {children}
  </div>
)

const TermButton: React.FC<
  React.ButtonHTMLAttributes<HTMLButtonElement> & { fullWidth?: boolean }
> = ({ className, fullWidth, children, ...props }) => (
  <button
    {...props}
    className={cn(
      'font-mono text-[11px] uppercase tracking-widest',
      'border-2 border-acc px-16 py-8',
      'hover:bg-acc hover:text-bac transition-colors',
      'disabled:opacity-30 disabled:pointer-events-none',
      fullWidth && 'w-full',
      className
    )}
  >
    {children}
  </button>
)

const TermInput: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({
  className,
  ...props
}) => (
  <input
    {...props}
    className={cn(
      'font-mono text-[12px] bg-transparent border-2 border-acc/30',
      'px-8 py-6 placeholder:text-acc/30 focus:border-acc outline-none w-full',
      className
    )}
  />
)

const TermSelect: React.FC<
  React.SelectHTMLAttributes<HTMLSelectElement> & { options: string[] }
> = ({ className, options, ...props }) => (
  <select
    {...props}
    className={cn(
      'font-mono text-[12px] bg-bac border-2 border-acc/30',
      'px-8 py-6 focus:border-acc outline-none w-full uppercase',
      className
    )}
  >
    <option value="">SELECT</option>
    {options.map((o) => (
      <option key={o} value={o}>
        {o}
      </option>
    ))}
  </select>
)

const cadenceBadge = (cadence: RationCadence) => {
  const cls =
    cadence === 'MONTHLY'
      ? 'text-acc'
      : cadence === 'QUARTERLY'
      ? 'text-acc/60'
      : 'text-acc/40'
  return <span className={cn('font-mono text-[11px]', cls)}>{cadence}</span>
}

const LedgerRow: React.FC<{ item: RationItem; isLast: boolean }> = ({
  item,
  isLast,
}) => (
  <div
    className={cn(
      'grid font-mono text-[12px] phone:text-[13px] py-[5px] gap-x-8',
      !isLast && 'border-b border-acc/10'
    )}
    style={{ gridTemplateColumns: '28px 1fr auto auto' }}
  >
    <span className="text-acc/40 tabular-nums">{item.line}</span>
    <span className="text-acc uppercase tracking-wide leading-snug">
      {item.nomenclature}
    </span>
    <span className="text-acc/50 text-right whitespace-nowrap">{item.spec}</span>
    <span className="text-right w-[80px] phone:w-[90px]">
      {cadenceBadge(item.cadence)}
    </span>
  </div>
)

const Ledger: React.FC = () => {
  const categories: { key: RationItem['category']; label: string }[] = [
    { key: 'NUTRITION', label: 'NUTRITION' },
    { key: 'HEALTH', label: 'HEALTH' },
    { key: 'HYGIENE', label: 'HYGIENE' },
    { key: 'EQUIPMENT', label: 'EQUIPMENT' },
  ]
  return (
    <>
      <SectionLabel>SECTION 1 — RATION MANIFEST ({RATION_COUNT} ITEMS)</SectionLabel>
      <div
        className="grid font-mono text-[10px] uppercase tracking-widest text-acc/40 pb-6 border-b-2 border-acc/20 mb-4 gap-x-8"
        style={{ gridTemplateColumns: '28px 1fr auto auto' }}
      >
        <span>NO.</span>
        <span>NOMENCLATURE</span>
        <span className="text-right">SPEC</span>
        <span className="text-right w-[80px] phone:w-[90px]">CADENCE</span>
      </div>
      {categories.map(({ key, label }) => {
        const items = RATION_MANIFEST.filter((i) => i.category === key)
        return (
          <div className="mb-12" key={key}>
            <div className="font-mono text-[10px] uppercase tracking-widest text-acc/30 pb-4">
              {label} ({items.length})
            </div>
            {items.map((item, idx, arr) => (
              <LedgerRow key={item.line} item={item} isLast={idx === arr.length - 1} />
            ))}
          </div>
        )
      })}
    </>
  )
}

const DoctrineBlock: React.FC = () => (
  <div className="mb-16 flex flex-col gap-y-4">
    {DOCTRINE_LINES.map((line, i) => (
      <p key={i} className="font-mono text-[12px] phone:text-[13px] text-acc leading-snug">
        {line}
      </p>
    ))}
    <p className="font-mono text-[13px] phone:text-[15px] text-acc font-bold mt-8 tracking-wide">
      {PRICE_LINE}
    </p>
  </div>
)

const Header: React.FC = () => (
  <>
    <div className="flex items-baseline justify-between mb-2">
      <h1 className="font-mono text-[13px] phone:text-[14px] uppercase tracking-[0.15em] text-acc font-bold">
        BASICS
      </h1>
      <span className="font-mono text-[11px] text-acc/40 tracking-widest">
        {MANUAL_REF}
      </span>
    </div>
    <HeavyRule className="mb-16" />
  </>
)

// ─── manifest card (M3 — print) ────────────────────────────────────────────

const ManifestCard: React.FC<{
  cycleNumber: number
  itemCount: number
  nextIssueAt?: string
}> = ({ cycleNumber, itemCount, nextIssueAt }) => (
  <div className="hidden print:block font-mono text-[12px] p-24 border-2 border-black">
    <div className="text-[14px] font-bold uppercase tracking-widest mb-8">
      LOT® BASIC — ISSUE MANIFEST
    </div>
    <div>{MANUAL_REF} · CYCLE {String(cycleNumber).padStart(3, '0')}</div>
    <div>LINES DISPATCHED: {itemCount}</div>
    {nextIssueAt && <div>NEXT ISSUE: {new Date(nextIssueAt).toUTCString().slice(0, 16)}</div>}
    <div className="mt-16 border-t border-black pt-8">ISSUED. NOT SOLD.</div>
  </div>
)

// ─── status / state-machine panel (in-app only) ────────────────────────────

const BASICS_SIZE_OPTIONS: BasicsSize[] = ['S', 'M', 'L', 'XL']

const StatusPanel: React.FC = () => {
  const me = useStore(stores.me)
  const isLoggedIn = useStore(stores.isLoggedIn)
  const { data, refetch } = useBasicsState({ enabled: isLoggedIn })
  const enroll = useBasicsEnroll()
  const confirm = useBasicsConfirm()
  const standDown = useBasicsStandDown()
  const dispatch = useBasicsDispatchIssue()

  const [size, setSize] = React.useState<BasicsSize | ''>('')
  const [address, setAddress] = React.useState('')

  const isUsership = me?.tags?.some((t) => t.toLowerCase() === 'usership') ?? false
  const basics = data?.basics
  const status = basics?.status ?? 'NONE'

  const planLabel = !isLoggedIn
    ? 'NOT LOGGED IN'
    : status === 'STEADY_STATE'
    ? 'BASIC / STEADY STATE'
    : status === 'ON_STRENGTH'
    ? 'BASIC / ON STRENGTH'
    : status === 'PENDING'
    ? 'BASIC / PENDING'
    : isUsership
    ? 'USERSHIP / AI'
    : 'NONE'

  const rationLabel =
    status === 'STEADY_STATE' || status === 'ON_STRENGTH'
      ? `ACTIVE — CYCLE ${String(basics?.cycleNumber ?? 0).padStart(3, '0')}${
          basics?.nextIssueAt
            ? ` — NEXT ISSUE ${new Date(basics.nextIssueAt).toISOString().slice(0, 10)}`
            : ''
        }`
      : status === 'PENDING'
      ? 'ROSTER INTAKE SUBMITTED — AWAITING CONFIRMATION'
      : 'NOT ON STRENGTH'

  const busy =
    enroll.isLoading || confirm.isLoading || standDown.isLoading || dispatch.isLoading

  return (
    <>
      <ThinRule className="mb-16 mt-8" />
      <SectionLabel>SECTION 2 — STATUS</SectionLabel>
      <div className="mb-16 font-mono text-[12px] phone:text-[13px] flex flex-col gap-y-4">
        <div className="grid gap-x-16" style={{ gridTemplateColumns: '120px 1fr' }}>
          <span className="text-acc/50 uppercase tracking-wider">PLAN</span>
          <span className="text-acc uppercase">{planLabel}</span>
        </div>
        <div className="grid gap-x-16" style={{ gridTemplateColumns: '120px 1fr' }}>
          <span className="text-acc/50 uppercase tracking-wider">RATION</span>
          <span className="text-acc/60 uppercase">{rationLabel}</span>
        </div>
      </div>

      {!isLoggedIn && (
        <p className="font-mono text-[11px] text-acc/40 mb-16">
          LOG IN TO ENROLL OR MANAGE ROSTER STATUS.
        </p>
      )}

      {isLoggedIn && status === 'NONE' && (
        <div className="border-2 border-acc/20 p-16 flex flex-col gap-y-8">
          <div className="text-acc/50 uppercase tracking-widest text-[10px]">
            UPGRADE PATH — USERSHIP → BASIC
          </div>
          {!isUsership ? (
            <div className="font-mono text-[12px] text-acc leading-snug">
              Requires USERSHIP / AI plan as base layer.
            </div>
          ) : (
            <>
              <div className="font-mono text-[12px] text-acc leading-snug">
                USERSHIP AI confirmed. BASIC ration available as additive layer
                (+{PRICE_LINE}).
              </div>
              <div className="flex flex-col gap-y-8 mt-8">
                <TermSelect
                  aria-label="Size"
                  options={BASICS_SIZE_OPTIONS}
                  value={size}
                  onChange={(e) => setSize(e.target.value as BasicsSize)}
                />
                <TermInput
                  placeholder="SHIPPING ADDRESS"
                  value={address}
                  onChange={(e) => setAddress(e.target.value)}
                />
                <TermButton
                  disabled={!size || !address.trim() || busy}
                  onClick={() =>
                    enroll.mutate(
                      { size: size as BasicsSize, shippingAddress: address },
                      { onSuccess: () => refetch() }
                    )
                  }
                >
                  SUBMIT ROSTER INTAKE
                </TermButton>
              </div>
            </>
          )}
        </div>
      )}

      {isLoggedIn && status === 'PENDING' && (
        <div className="border-2 border-acc/20 p-16 flex flex-col gap-y-8">
          <div className="text-acc/50 uppercase tracking-widest text-[10px]">
            ROSTER INTAKE — {basics?.size} — {basics?.shippingAddress}
          </div>
          <div className="flex gap-x-8">
            <TermButton
              disabled={busy}
              onClick={() => confirm.mutate(undefined, { onSuccess: () => refetch() })}
            >
              CONFIRM — GO ON STRENGTH
            </TermButton>
            <TermButton
              disabled={busy}
              onClick={() => standDown.mutate(undefined, { onSuccess: () => refetch() })}
            >
              CANCEL INTAKE
            </TermButton>
          </div>
        </div>
      )}

      {isLoggedIn && (status === 'ON_STRENGTH' || status === 'STEADY_STATE') && (
        <div className="border-2 border-acc/20 p-16 flex flex-col gap-y-8">
          <div className="text-acc/50 uppercase tracking-widest text-[10px]">
            ROSTER — {basics?.size} — {basics?.shippingAddress}
          </div>
          <div className="flex flex-wrap gap-8">
            <TermButton
              disabled={busy}
              onClick={() => dispatch.mutate(undefined, { onSuccess: () => refetch() })}
            >
              DISPATCH NEXT ISSUE
            </TermButton>
            <TermButton disabled={busy} onClick={() => window.print()}>
              PRINT MANIFEST CARD
            </TermButton>
            <TermButton
              disabled={busy}
              onClick={() => standDown.mutate(undefined, { onSuccess: () => refetch() })}
            >
              STAND DOWN
            </TermButton>
          </div>
          <div className="font-mono text-[10px] text-acc/30 mt-4">
            DISPATCH IS A MANUAL FULFILLMENT TRIGGER. SUPPLIER/COURIER API NOT YET WIRED.
          </div>
        </div>
      )}

      {data?.issues && data.issues.length > 0 && (
        <div className="mt-16">
          <SectionLabel>SECTION 3 — ISSUE LOG</SectionLabel>
          {data.issues
            .slice()
            .reverse()
            .map((issue) => (
              <div
                key={issue.cycleNumber}
                className="grid font-mono text-[11px] py-4 border-b border-acc/10 gap-x-8"
                style={{ gridTemplateColumns: '80px 1fr auto' }}
              >
                <span className="text-acc/40">CYCLE {String(issue.cycleNumber).padStart(3, '0')}</span>
                <span className="text-acc/60">{issue.itemLines.length} LINES DISPATCHED</span>
                <span className="text-acc/40">
                  {new Date(issue.dispatchedAt).toISOString().slice(0, 10)}
                </span>
              </div>
            ))}
        </div>
      )}

      <ManifestCard
        cycleNumber={basics?.cycleNumber ?? 0}
        itemCount={data?.issues?.[data.issues.length - 1]?.itemLines.length ?? 0}
        nextIssueAt={basics?.nextIssueAt}
      />
    </>
  )
}

// ─── main component ────────────────────────────────────────────────────────

type BasicsProps = {
  standalone?: boolean
}

export const Basics: React.FC<BasicsProps> = ({ standalone = false }) => {
  useDocumentTitle('Basics')
  const isMirrorOn = useStore(stores.isMirrorOn)

  const content = (
    <>
      <Header />
      <DoctrineBlock />
      <ThinRule className="mb-16" />
      <Ledger />
      {!standalone && <StatusPanel />}
    </>
  )

  if (standalone) {
    return (
      <div className="min-h-screen bg-bac text-acc px-16 phone:px-32 tablet:px-48 desktop:px-64 py-24 phone:py-32 max-w-3xl mx-auto">
        {content}
      </div>
    )
  }

  return (
    <div
      className={cn(
        'min-h-[100dvh] flex flex-col',
        'px-16 phone:px-32 tablet:px-48 desktop:px-64',
        'pt-24 phone:pt-32 pb-[120px]',
        isMirrorOn && 'text-white'
      )}
    >
      {content}
    </div>
  )
}
