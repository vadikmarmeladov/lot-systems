    ...
    for (const trigger of fresh as LogTrigger[]) {
      if (trigger === 'toggle-synth') {
        ...
        const lines = [
          ...
          '/story [win]  Compressed story — day (default), week, month, year',
        ...
        if (!storyLoading) {
          ...
          try {
            // /story [day|week|month|year] — compression window, defaults to day.
            const periodMatch = value.match(/\/story\s+(day|today|week|month|year)\b/i)
            const period = (periodMatch
              ? periodMatch[1].toLowerCase().replace('today', 'day')
