          ...
          return (
            <LogContainer key={id} log={log} dateFormat={dateFormat}>
              <Block label={`CAL [${entryType.toUpperCase()}]:`} blockView>
                <div className="uppercase tracking-widest">{text}</div>
                <div className="opacity-40 mt-8 tabular-nums">
                  {dateLabel}{time ? ` · ${time}` : ''}
                </div>
                <div className="opacity-30 uppercase tracking-widest mt-4">
                  {isElapsed ? 'ELAPSED' : 'SCHEDULED'}
                </div>
              </Block>
            </LogContainer>
          )
