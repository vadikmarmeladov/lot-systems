  ...
  return (
    ...
    >
      ...
      {pastLogIds.map((id) => {
        ...
        } else if (log.event === 'calendar_entry_status') {
          const status = log.metadata?.status as string | undefined
          if (!status) return null
          return (
            <LogContainer key={id} log={log} dateFormat={dateFormat}>
              <Block label="CAL-STAT:" blockView>
                <div className="uppercase tracking-widest opacity-60">{status}</div>
              </Block>
            </LogContainer>
          )
        } else if (log.event === 'calendar_event_fired') {
          const date = log.metadata?.date as string | undefined
          const time = log.metadata?.time as string | undefined
          return (
            <LogContainer key={id} log={log} dateFormat={dateFormat}>
              <Block label="CAL-ALERT:" blockView>
                <div className="uppercase tracking-widest">[ FIRED ]</div>
                {date && (
                  <div className="opacity-40 mt-8 tabular-nums">
                    {date}{time && ` @ ${time}`}
                  </div>
