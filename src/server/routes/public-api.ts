...
async function performHealthChecks(): Promise<{
  ...
  // Determine overall status: critical checks (Database + Memory Engine) failing = error,
  // non-critical checks failing = degraded, all ok = ok
  const criticalNames = new Set(['Database stack', 'Memory Engine'])
  const hasCriticalError = checks.some(
    (c) => c.status === 'error' && criticalNames.has(c.name)
  )
