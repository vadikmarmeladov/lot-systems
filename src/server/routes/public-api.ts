...
async function checkEngineStack(): Promise<SystemCheck> {
  const start = Date.now()
  try {
    // Check Weather API (used by multiple features)
...
async function performHealthChecks(): Promise<{
  ...
  // Determine overall status: error if any critical checks fail, degraded if non-critical
  const criticalChecks = ['Database stack', 'Authentication engine', 'Memory Engine']
