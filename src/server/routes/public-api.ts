...
export default async (fastify: FastifyInstance) => {
  ...
  fastify.get<{
    ...
    if (userIdOrUsername === 'machiavelli') {
      ...
      let usershipCount = 1
      try {
        totalUsers = (await models.User.count()) + 1 // +1 for demo account
        usershipCount = (await models.User.count({
          where: { tags: { [Op.contains]: ['Usership'] } }
        })) + 1 // +1 for demo
      } catch (e) {
        // ignore — demo defaults hold
      }
    ...
    try {
      // Try custom URL first (JSONB containment query — avoids full table scan)
      let user = await models.User.findOne({
        where: {
          metadata: { [Op.contains]: { privacy: { customUrl: userIdOrUsername } } },
        },
      })

      // Fall back to user ID lookup
      if (!user) {
        user = await models.User.findOne({ where: { id: userIdOrUsername } })
      }
      ...
      if (hasUsershipTag) {
        try {
          ...
          const usershipMembers = await models.User.findAll({
            where: { tags: { [Op.contains]: ['Usership'] } },
