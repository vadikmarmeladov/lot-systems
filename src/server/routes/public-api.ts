...
async function checkAuth(): Promise<SystemCheck> {
  ...
  try {
    ...
    // Check Resend API is configured (required for email-based login)
...
async function checkSettings(): Promise<SystemCheck> {
  const start = Date.now()
  try {
    // Check PWA manifest exists (required for install + notifications)
    ...
    // Check service worker exists (required for offline support)
    ...
    // Check email sender is configured (required for notification emails)
    if (!config.email?.fromEmail) {
      return {
        name: 'Settings',
        status: 'error',
        message: 'Notification sender email not configured',
        duration: Date.now() - start,
      }
    }
...
async function performHealthChecks(): Promise<{
  ...
  // Critical services: database and authentication must be fully operational
  ...
  return {
    ...
    checks,
    overall,
    criticalServices: [...criticalNames],
  }
}
