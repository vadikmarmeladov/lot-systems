...
${recentPrayers.length > 0 ? `RECENT SCRIPTURES (DO NOT REPEAT):\n${recentPrayers.join('\n')}` : ''}`
  ...
  // widget data. Period selects the compression window: day/week/month/year
  // each pull a wider log range and target a proportionally longer narrative.
  // ============================================================================
  const STORY_PERIODS = ['day', 'week', 'month', 'year'] as const
  type StoryPeriod = (typeof STORY_PERIODS)[number]
  const STORY_PERIOD_CONFIG: Record<StoryPeriod, { logLimit: number; words: string; label: string }> = {
    day: { logLimit: 100, words: '100-200', label: 'the last day' },
    week: { logLimit: 400, words: '150-280', label: 'the last 7 days' },
    month: { logLimit: 1200, words: '220-350', label: 'the last 30 days' },
    year: { logLimit: 4000, words: '320-500', label: 'the last 365 days' },
  }
  ...
  fastify.post(
    ...
    async (
      ...
      const since = dayjs().subtract(1, period === 'day' ? 'day' : period === 'week' ? 'week' : period === 'month' ? 'month' : 'year').toDate()

      const logs = await fastify.models.Log.findAll({
        where: { userId: req.user.id, createdAt: { [Op.gte]: since } },
        order: [['createdAt', 'DESC']],
        limit: periodConfig.logLimit,
      })

      // Sample size scales with the compression window — a year story draws
      // on more entries than a day story, within a bounded prompt budget.
      const sampleSize = { day: 5, week: 8, month: 12, year: 16 }[period]
...
The operator typed a log entry and invoked /story ${period}. Your task: write a story compressing ${periodConfig.label} (${periodConfig.words} words) that reflects their journey, mood trajectory, and self-care patterns over that window. The story should feel personal, grounded, and real — not generic motivational writing. A day story is a close-up moment; a year story is a wide arc — write at the altitude the period implies.
...
- Target length: ${periodConfig.words} words.`

      const dataBlock = `
OPERATOR LOG ENTRY: "${logText || '(no text)'}"
COMPRESSION WINDOW: ${period.toUpperCase()} (${periodConfig.label})
...
${selfCareNotes.map(n => `- ${n}`).join('\n') || '- (none)'}`
      ...
      try {
        ...
        const story = await engine.generateCompletion(fullPrompt, period === 'year' ? 900 : period === 'month' ? 700 : 512)
