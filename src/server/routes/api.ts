    ...
    async (
      ...
      const period: 'day' | 'week' | 'month' | 'year' =
        (['day', 'week', 'month', 'year'] as const).includes(req.body.period as any)
          ...
          : 'day'
      const periodStart = dayjs().subtract(1, period).toDate()
      // Compression depth scales with the window — a year of entries
      // needs more source material and a longer narrative than a day.
      const entryCap = { day: 10, week: 20, month: 30, year: 40 }[period]
      const maxTokens = period === 'day' || period === 'week' ? 512 : 768
      const wordTarget = period === 'day' || period === 'week' ? '100-200' : '200-350'

      const logs = await fastify.models.Log.findAll({
        where: { userId: req.user.id, createdAt: { [Op.gte]: periodStart } },
        order: [['createdAt', 'DESC']],
        limit: 500,
      })

      const recentEntries = logs
        .filter(l => l.event === 'note' && l.text && l.text.length > 0)
        .slice(0, entryCap)
...
The operator typed a log entry and invoked /story ${period === 'day' ? '' : period}. Your task: write a compressed story of their ${period} (${wordTarget} words) that reflects their journey, mood trajectory, and self-care patterns across that window. The story should feel personal, grounded, and real — not generic motivational writing.
...
- Keep it within the word target.`
      ...
      const dataBlock = `
OPERATOR LOG ENTRY: "${logText || '(no text)'}"
COMPRESSION WINDOW: last 1 ${period}
