...
${recentPrayers.length > 0 ? `RECENT SCRIPTURES (DO NOT REPEAT):\n${recentPrayers.join('\n')}` : ''}`
  ...
  // Generates a compressed narrative from the operator's own log window,
  // moods, and self-care events. The window and target length scale with
  // the requested period — this is the LOG -> COMPRESS step of the LOT AI
  // compression loop (see docs/corporate/LOT-AI-PRODUCT-BRIEF.md).
  // ============================================================================
  const STORY_PERIODS: Record<
    'day' | 'week' | 'month' | 'year',
    { days: number; label: string; words: string; sampleSize: number; maxTokens: number }
  > = {
    day: { days: 1, label: 'the last day', words: '100-200', sampleSize: 10, maxTokens: 512 },
    week: { days: 7, label: 'the last week', words: '150-250', sampleSize: 20, maxTokens: 600 },
    month: { days: 30, label: 'the last month', words: '200-300', sampleSize: 35, maxTokens: 700 },
    year: { days: 365, label: 'the last year', words: '250-350', sampleSize: 50, maxTokens: 800 },
  }
  ...
  fastify.post(
    ...
    async (
      ...
      const period: 'day' | 'week' | 'month' | 'year' =
        req.body.period && STORY_PERIODS[req.body.period] ? req.body.period : 'day'
      const cfg = STORY_PERIODS[period]

      const logs = await fastify.models.Log.findAll({
        where: {
          userId: req.user.id,
          createdAt: { [Op.gte]: dayjs().subtract(cfg.days, 'day').toDate() },
        },
        order: [['createdAt', 'DESC']],
        limit: Math.max(200, cfg.sampleSize * 10),
      })

      const recentEntries = logs
        .filter(l => l.event === 'note' || l.event === 'log_entry' || l.event === 'journal')
        .slice(0, cfg.sampleSize)
        .map(l => (l.text || '').substring(0, 200))
        .filter(Boolean)

      const moodLogs = logs.filter(l => l.event === 'emotional_checkin').slice(0, cfg.sampleSize)
      const recentMoods = moodLogs.map(l => (l.metadata?.emotionalState as string || '').toUpperCase()).filter(Boolean)
      ...
      const sampleCap = Math.min(cfg.sampleSize, 12)
...
The operator typed a log entry and invoked /story for ${cfg.label}. Your task: write a compressed story (${cfg.words} words) that reflects their journey, mood trajectory, and self-care patterns over ${cfg.label} — not just the single entry they typed. The story should feel personal, grounded, and real — not generic motivational writing.

RULES:
- Write in second person ("You...")
- Draw from their actual log entries, moods, and self-care answers below
- Reference specific details from their data — make it feel like THEIR story, compressed
...
- Keep it within ${cfg.words} words.`
      ...
      const dataBlock = `
OPERATOR LOG ENTRY: "${logText || '(no text)'}"
PERIOD: ${period.toUpperCase()} (${cfg.label})
