...
<body>
  ...
  fastify.get('/export/training-data', async (req, reply) => {
    ...
    try {
      ...
      console.error('Training data export failed:', error.message)
      return reply.status(500).send({ error: 'Export failed' })
    }
  })

  // ==========================================================================
  // LOT® API — Robot Data Integration
  // Authenticated endpoints for uploading personal, spiritual, routine,
  // and chakra data to external robots, companions, and AI systems.
  // ==========================================================================

  fastify.get('/lot/connect', async (req, reply) => {
    const user = req.user
    const logs = await fastify.models.Log.findAll({
    ...
    const hasUsership = (user.tags || []).some(
      (t: string) => t.toLowerCase() === 'usership'
    )
    ...
    return {
      ...
      capabilities: {
        personal: true,
        spiritual: hasUsership,
        routine: true,
        chakra: hasUsership,
        training_export: hasUsership,
      },
      data_summary: {
      ...
      endpoints: {
        upload_personal: 'POST /api/lot/upload/personal',
        upload_spiritual: 'POST /api/lot/upload/spiritual',
        upload_routine: 'POST /api/lot/upload/routine',
        upload_chakra: 'POST /api/lot/upload/chakra',
        export_training: 'GET /api/export/training-data',
      },
      api_version: '1.0',
      system: 'LOT® Systems',
    }
  ...
  fastify.post<{
  ...
  }>('/lot/upload/personal', async (req, reply) => {
    const body = req.body || {}
    const updates: Record<string, any> = {}

    if (body.firstName !== undefined) updates.firstName = String(body.firstName).slice(0, 100)
    if (body.lastName !== undefined) updates.lastName = String(body.lastName).slice(0, 100)
    if (body.city !== undefined) updates.city = String(body.city).slice(0, 100)
    if (body.country !== undefined) updates.country = String(body.country).slice(0, 10)
    if (body.timeZone !== undefined) updates.timeZone = String(body.timeZone).slice(0, 64)

    if (!Object.keys(updates).length && !body.metadata) {
      return reply.throw.badParams('No personal data fields provided')
    }
    ...
    if (body.metadata && typeof body.metadata === 'object') {
      updates.metadata = { ...(user.metadata || {}), ...body.metadata }
    }
    ...
    await fastify.models.Log.create({
      userId: req.user.id,
      text: `Personal data updated via LOT® API: ${Object.keys(updates).join(', ')}`,
      event: 'lot_api_upload',
      metadata: { category: 'personal', fields: Object.keys(updates) },
      context,
    })
  ...
  fastify.post<{
    Body: {
      type: 'prayer' | 'intention' | 'scripture' | 'meditation' | 'reflection'
      text: string
      metadata?: Record<string, any>
    }
  }>('/lot/upload/spiritual', async (req, reply) => {
    const VALID_TYPES = ['prayer', 'intention', 'scripture', 'meditation', 'reflection']
    const type = req.body?.type
    const text = (req.body?.text || '').trim().slice(0, MAX_LOG_TEXT_LENGTH)

    if (!type || !VALID_TYPES.includes(type)) {
      return reply.throw.badParams(`type must be one of: ${VALID_TYPES.join(', ')}`)
    }
    if (!text) {
      return reply.throw.badParams('text is required')
    }
  ...
  }>('/lot/upload/routine', async (req, reply) => {
    const VALID_STATUS = ['completed', 'skipped', 'scheduled']
    const activity = (req.body?.activity || '').trim().slice(0, 500)
    const status = req.body?.status

    if (!activity) {
      return reply.throw.badParams('activity is required')
    }
    if (!status || !VALID_STATUS.includes(status)) {
      return reply.throw.badParams(`status must be one of: ${VALID_STATUS.join(', ')}`)
    }

    const eventMap: Record<string, string> = {
      completed: 'self_care_complete',
      skipped: 'self_care_skip',
      scheduled: 'lot_api_routine_scheduled',
    }

    const context = await getLogContext(req.user)
    const log = await fastify.models.Log.create({
      userId: req.user.id,
      text: `${status === 'completed' ? 'Self-care completed' : status === 'skipped' ? 'Self-care skipped' : 'Routine scheduled'}: ${activity}`,
      event: eventMap[status],
      metadata: {
        category: 'routine',
        activity,
        status,
        protocol: req.body.protocol?.slice(0, 200) || null,
        durationMinutes: typeof req.body.duration_minutes === 'number'
          ? Math.min(Math.max(0, req.body.duration_minutes), 1440)
          : null,
        notes: req.body.notes?.slice(0, 1000) || null,
        ...(req.body.metadata || {}),
      },
      context,
    })
  ...
  const VALID_CHAKRA_IDS = ['crown', 'thirdEye', 'throat', 'heart', 'solarPlexus', 'sacral', 'root'] as const
  const VALID_CHAKRA_STATUSES = ['blocked', 'strained', 'depleted', 'balanced', 'open', 'overactive', 'grounded', 'flowing'] as const
  ...
  fastify.post<{
  ...
  }>('/lot/upload/chakra', async (req, reply) => {
    const chakras = req.body?.chakras
    if (!Array.isArray(chakras) || chakras.length === 0) {
      return reply.throw.badParams('chakras array is required with at least one entry')
    }
    if (chakras.length > 7) {
      return reply.throw.badParams('maximum 7 chakras')
    }

    const validated = []
    for (const c of chakras) {
      if (!c.id || !(VALID_CHAKRA_IDS as readonly string[]).includes(c.id)) {
        return reply.throw.badParams(`invalid chakra id: ${c.id}. Valid: ${VALID_CHAKRA_IDS.join(', ')}`)
      }
      if (typeof c.charge !== 'number' || c.charge < 0 || c.charge > 100) {
        return reply.throw.badParams(`charge must be 0-100 for chakra ${c.id}`)
      }
      if (!c.status || !(VALID_CHAKRA_STATUSES as readonly string[]).includes(c.status)) {
        return reply.throw.badParams(`invalid status for ${c.id}. Valid: ${VALID_CHAKRA_STATUSES.join(', ')}`)
      }
      validated.push({
        id: c.id,
        charge: Math.round(c.charge),
        status: c.status,
      })
    }

    const session = req.body.session ? {
      durationMinutes: typeof req.body.session.durationMinutes === 'number'
        ? Math.min(Math.max(0, req.body.session.durationMinutes), 1440)
        : null,
      coherence: typeof req.body.session.coherence === 'number'
        ? Math.min(Math.max(0, req.body.session.coherence), 100)
        : null,
      interactionCount: typeof req.body.session.interactionCount === 'number'
        ? Math.max(0, Math.round(req.body.session.interactionCount))
        : null,
    } : null
    ...
    const log = await fastify.models.Log.create({
      userId: req.user.id,
      text: `Chakra data: ${validated.map(c => `${c.id}=${c.charge}% (${c.status})`).join(', ')}`,
      event: 'lot_api_chakra',
      metadata: {
        category: 'chakra',
        chakras: validated,
        session,
    ...
    return {
      uploaded: true,
      category: 'chakra',
      chakras_recorded: validated.length,
      logId: log.id,
      timestamp: new Date().toISOString(),
    }
  })
