...
${recentPrayers.length > 0 ? `RECENT SCRIPTURES (DO NOT REPEAT):\n${recentPrayers.join('\n')}` : ''}`
  ...
  fastify.post(
    ...
    async (
      ...
      // Compression window: how far back this period's story reaches, and how many
      // entries per bucket it draws from — a year compresses months, not lines.
      const PERIOD_CONFIG: Record<typeof period, { days: number; entryCount: number; label: string }> = {
        day: { days: 1, entryCount: 8, label: "today" },
        week: { days: 7, entryCount: 10, label: 'this week' },
        month: { days: 31, entryCount: 15, label: 'this month' },
        year: { days: 366, entryCount: 20, label: 'this year' },
      }
      const { days: windowDays, entryCount, label: periodLabel } = PERIOD_CONFIG[period]
      ...
      const moodLogs = logs.filter(l => l.event === 'emotional_checkin').slice(0, entryCount)
      const recentMoods = moodLogs.map(l => (l.metadata?.emotionalState as string || '').toUpperCase()).filter(Boolean)
      ...
      const PERIOD_WORD_TARGET: Record<typeof period, string> = {
        day: '60-120 words, 1 short paragraph',
        week: '100-200 words, 1-2 paragraphs',
        month: '150-250 words, 2-3 paragraphs — compress the arc, not the log',
        year: '200-320 words, 2-4 paragraphs — this is a year compressed to its shape, not a list of months',
      }

      const systemPrompt = `You are the Story module of LOT Systems — a personal operating system that compresses the operator's data from a given period into a short narrative.

The operator typed a log entry and invoked /story ${period !== 'day' ? period : ''}. Your task: write a compressed story covering ${periodLabel} (${PERIOD_WORD_TARGET[period]}) that reflects their journey, mood trajectory, and self-care patterns across that window. The story should feel personal, grounded, and real — not generic motivational writing.
...
- The wider the period, the more you compress: a year should read as a shape, not a chronicle
...
PERIOD: ${period.toUpperCase()} (${periodLabel}, ${logs.length} log${logs.length !== 1 ? 's' : ''} in window)
