...
${recentPrayers.length > 0 ? `RECENT SCRIPTURES (DO NOT REPEAT):\n${recentPrayers.join('\n')}` : ''}`
  ...
  const STORY_PERIOD_CONFIG: Record<
    StoryPeriod,
    { days: number; limit: number; entrySample: number; wordTarget: string; label: string }
  > = {
    day: { days: 1, limit: 200, entrySample: 5, wordTarget: '100-200', label: 'DAY' },
    week: { days: 7, limit: 400, entrySample: 8, wordTarget: '150-250', label: 'WEEK' },
    month: { days: 30, limit: 800, entrySample: 12, wordTarget: '200-300', label: 'MONTH' },
    year: { days: 365, limit: 2000, entrySample: 15, wordTarget: '250-350', label: 'YEAR' },
  }
  ...
  fastify.post(
    ...
    async (
      ...
      const period: StoryPeriod = STORY_PERIODS.includes(req.body.period as StoryPeriod)
        ...
        : 'day'
      const { days, limit, entrySample, wordTarget, label } = STORY_PERIOD_CONFIG[period]
      ...
      const activeDays = new Set(logs.map(l => l.createdAt.toISOString().slice(0, 10))).size
      ...
      const moodLogs = logs.filter(l => l.event === 'emotional_checkin').slice(0, entrySample)
      const recentMoods = moodLogs.map(l => (l.metadata?.emotionalState as string || '').toUpperCase()).filter(Boolean)
      ...
      const periodInstruction = period === 'day'
        ? 'The operator invoked /story. Write a story reflecting today and their most recent journey.'
        : `The operator invoked /story ${period}. This is a COMPRESSED look back across their last ${label.toLowerCase()} — synthesize the arc across ${activeDays} active day(s), not a single moment. Name the pattern across the period, not just the latest entry.`
...
Your task: write 1-2 paragraphs (${wordTarget} words) that reflect their journey, mood trajectory, and self-care patterns over this period. The story should feel personal, grounded, and real — not generic motivational writing.
...
- Keep it within ${wordTarget} words.`

      const dataBlock = `
PERIOD: ${label} (${activeDays} active day(s) of data, ${logs.length} entries)
...
${selfCareNotes.map(n => `- ${n}`).join('\n') || '- (none)'}`
      ...
      try {
        ...
        const story = await engine.generateCompletion(fullPrompt, period === 'year' ? 768 : 512)
