 ...
 *
 * Shared between client (OPEN TAB ledger render) and server (issue engine)
 * so the item list and cadence logic never drift between the two.
 */
...
// Quarterly items ship on the 1st, 4th, 7th... issue (every 3rd cycle).
// Annual items (none currently loaded — cadence reserved for future items)
// ship on the 1st, 13th, 25th... issue (every 12th cycle).
export function itemsForIssue(issueNumber: number): RationItem[] {
  const shipsQuarterly = (issueNumber - 1) % 3 === 0
  const shipsAnnually = (issueNumber - 1) % 12 === 0
  return RATION_MANIFEST.filter((item) => {
    if (item.cadence === 'MONTHLY') return true
    if (item.cadence === 'QUARTERLY') return shipsQuarterly
    return shipsAnnually
  })
}

export type BasicsStatus = 'NONE' | 'PENDING' | 'ON_STRENGTH' | 'STEADY_STATE' | 'STAND_DOWN'
...
export type BasicsChargeRecord = {
  date: string
  amountUsd: number
  status: 'SIMULATED'
}
...
export type BasicsState = {
  ...
  chargeHistory: BasicsChargeRecord[]
  issueLog: BasicsIssueRecord[]
  nextIssueAt: string | null
}

export const BASICS_STATE_DEFAULT: BasicsState = {
  status: 'NONE',
  roster: null,
  enrolledAt: null,
  onStrengthAt: null,
