 ...
 *
 * Public data. COGS withheld — see src/server/basics/cogs.ts (server-only).
 */
...
// COGS ceiling: USD 40.00 landed. Margin floor: 60%. (withheld from this file — see server/basics/cogs.ts)
...
// ─── SECTION 3 — month-by-month load engine ───────────────────────────────
// cycleNumber is 0-indexed: 0 = first issue dispatched.
// MONTHLY items ship every cycle. QUARTERLY ship on cycles 0, 3, 6, ...
// ANNUALLY ship on cycles 0, 12, 24, ... (none in the current 23-item load).
export function computeIssueLoad(cycleNumber: number): RationItem[] {
  return RATION_MANIFEST.filter((item) => {
    if (item.cadence === 'MONTHLY') return true
    if (item.cadence === 'QUARTERLY') return cycleNumber % 3 === 0
    return cycleNumber % 12 === 0
  })
}

// STEADY STATE reached once a full quarterly rhythm has completed (3 cycles).
export const STEADY_STATE_CYCLE_THRESHOLD = 3
